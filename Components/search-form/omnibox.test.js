...
import EOmniboxSearchForm from './omnibox.vue';
...
/**
 * Since the omnibox extends the search-form, it should inherit
 * all its functionality.
 * These tests are copied from the search-form tests to ensure that the omnibox component passes all
 * the same tests as the search-form.
 */
...
describe('EOmniboxSearchForm', () => {
  ...
  it('renders and is visible', () => {
    const wrapper = shallowMount(EOmniboxSearchForm, options);
  ...
  describe('With autoSuggest is 1', () => {
    ...
    describe('experiment: debounce delay', () => {
      it('uses regular delay when experiment is disabled', async () => {
        ...
        await mount(EOmniboxSearchForm, options);
    ...
    it.each([
      ...
      const wrapper = mount(EOmniboxSearchForm, options);
      wrapper.setData({
  ...
  it('renders hidden inputs with hidden-inputs', () => {
    ...
    const wrapper = shallowMount(EOmniboxSearchForm, {
      ...options,
      propsData: {
        ...options.propsData,
        hiddenInputs,
  ...
  it('renders with a pre-filled query if prefill-query is enabled', () => {
    const wrapper = mount(EOmniboxSearchForm, {
      ...options,
      propsData: {
        ...propsData,
        prefillQuery: true,
  ...
  describe('Autofocus', () => {
    ...
    it('autofocuses the input field if prop is true and consent is given', async () => {
      userStore.$patch({ consentGiven: true });
      const wrapper = await mount(EOmniboxSearchForm, {
        ...options,
        propsData: { ...propsData, hasAutoFocus: true },
      });
      const input = wrapper.find(byTestId('search-form-input'));
      expect(input.vm.autofocus).toBe(true);
    });
  ...
  describe('Reset action', () => {
    it('resets input value and focuses it', async () => {
      const value = 'test';
      const wrapper = await mount(EOmniboxSearchForm, options);
  ...
  it('should have required aria-label by default to pass accessibility', () => {
    ...
    const searchFieldSearchInput = searchField.find('textarea');
  ...
  it('textafield should have aria-expanded=true when suggestion list is visible', async () => {
    const wrapper = mount(EOmniboxSearchForm, options);
    const searchField = wrapper.find(byTestId('search-form-search-field'));
    const searchFieldSearchInput = searchField.find('textarea');
